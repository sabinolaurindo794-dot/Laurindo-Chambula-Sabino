...
export const Home: React.FC<HomeProps> = ({
  ...
  return (
    <div className="flex flex-col h-[calc(100dvh-3.5rem)] px-3.5 py-2 max-w-md mx-auto w-full relative z-10 justify-between overflow-hidden text-xs select-none">
      {/* Top Header Bar */}
      <motion.div
      ...
      <motion.div
        ...
        animate={{ opacity: 1, y: 0 }}
        className="flex items-center gap-2 px-2.5 py-1.5 bg-[#120324] rounded-lg border border-[#c084fc]/15 shrink-0"
      >
        <div className="w-6 h-6 rounded-full bg-gradient-to-tr from-[#c084fc] to-[#7c3aed] flex items-center justify-center text-xs shrink-0 shadow-inner">
          {userProfile.av}
        </div>
        <div className="flex-1 min-w-0 flex items-center justify-between">
          <span className="font-syne font-bold text-xs text-[#f5f0ff] truncate">
            {currentUser}
          </span>
          <span className="text-[10px] text-[#fbbf24] font-medium flex items-center gap-0.5">
            <Sparkles size={10} />
      ...
      {!isOnline && (
        <div className="px-2 py-1 bg-[#f59e0b]/15 border border-[#f59e0b]/30 rounded-lg text-[10px] text-[#fef08a] flex items-center gap-1.5 shrink-0">
          <WifiOff size={12} className="shrink-0 text-[#fbbf24]" />
          <span className="truncate">Modo Offline · Pontos guardados no telemóvel</span>
        </div>
      )}

      {isOnline && pendingGamesCount > 0 && (
        <div className="px-2 py-1 bg-[#c084fc]/15 border border-[#c084fc]/30 rounded-lg text-[10px] text-[#f5f0ff] flex items-center justify-between shrink-0">
          <span className="flex items-center gap-1">
            <RefreshCw size={10} className={`text-[#c084fc] ${isSyncing ? 'animate-spin' : ''}`} />
            <strong>{pendingGamesCount}</strong> offline
          </span>
          <button
            onClick={handleManualSync}
            disabled={isSyncing || !token}
            className="px-2 py-0.5 rounded bg-[#c084fc] text-white text-[9px] font-bold cursor-pointer"
          >
            Sincronizar
          </button>
        </div>
      ...
      {deferredPrompt && (
        <div className="px-2 py-1 bg-[#7c3aed]/20 border border-[#c084fc]/40 rounded-lg text-[10px] text-[#f5f0ff] flex items-center justify-between shrink-0">
          <span className="flex items-center gap-1">
            <Download size={10} className="text-[#c084fc]" />
            <span>Instalar no ecrã principal</span>
          </span>
          <button
            onClick={handleInstallPWA}
            className="px-2 py-0.5 rounded bg-gradient-to-r from-[#c084fc] to-[#7c3aed] text-white text-[9px] font-bold cursor-pointer"
          >
            Instalar
          </button>
        </div>
      ...
      {/* Disciplines Grid Section (3x3 grid fitting in single view) */}
      <div className="shrink-0">
        <div className="flex items-center justify-between mb-1">
          <span className="text-[10px] uppercase font-bold tracking-wider text-[#a78bca]">
            Disciplinas & Categorias
          </span>
          <span className="text-[9px] text-[#c084fc] font-medium">
            {CATEGORIES[selectedCat]?.label || 'Todas'} selecionada
          </span>
        </div>

        <div className="grid grid-cols-3 gap-1.5">
          {(['todas', 'filosofia', 'ciencias', 'antropologia', 'historia', 'tecnologia', 'comunicacao', 'economia', 'medicina'] as CategoryId[]).map(
            (catKey) => {
              ...
              return (
                <button
                  ...
                  className={`p-1.5 rounded-lg border text-left transition-all duration-150 cursor-pointer flex items-center gap-1.5 h-11 relative overflow-hidden ${
                    isSelected
                      ? 'border-[#c084fc] bg-[#240847] shadow-sm shadow-[#c084fc]/20 ring-1 ring-[#c084fc]/50'
                      : 'border-[#c084fc]/20 bg-[#1a0533] hover:bg-[#1f073d]'
                  }`}
                >
                  <span className="text-base shrink-0">{cat.icon}</span>
                  <div className="min-w-0 flex-1">
                    <div className="font-syne font-bold text-[11px] text-[#f5f0ff] truncate leading-tight">
                      {cat.label}
                    </div>
                    <div className="text-[9px] text-[#a78bca] truncate">
                      {cat.count} {cat.count === 1 ? 'q' : 'questões'}
                    </div>
                  </div>
                </button>
              );
      ...
      {/* Game Mode */}
      <div className="shrink-0">
        <span className="block text-[10px] uppercase font-bold tracking-wider text-[#a78bca] mb-1">
          Modo de Jogo
        </span>
        ...
        <div className="grid grid-cols-3 gap-1.5">
          <button
            ...
            className={`p-1.5 rounded-lg border text-center transition-all cursor-pointer flex flex-col items-center justify-center gap-0.5 h-10 ${
              selectedMode === 'solo'
          ...
          >
            <Target size={14} />
            <span className="text-[10px] font-semibold">Solo</span>
          </button>
          ...
          >
            <Users size={14} />
            <span className="text-[10px] font-semibold">Multiplayer</span>
          </button>
        </div>
      </div>
      ...
      <button
        onClick={onStartGame}
        className="w-full py-2.5 rounded-xl bg-gradient-to-r from-[#c084fc] to-[#7c3aed] text-white font-syne font-bold text-xs shadow-lg shadow-[#7c3aed]/25 hover:brightness-110 active:scale-[0.98] transition-all cursor-pointer shrink-0"
      >
